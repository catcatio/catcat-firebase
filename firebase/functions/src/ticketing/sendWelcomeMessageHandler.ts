const welcomeMessage = (languageCode, formatter) => languageCode === 'th'
  ? formatter.quickReplyTemplate('ดีจ้า มีอะไรให้ช่วยมั๊ย?', 'มีงานอีเว้นท์อะไรบ้าง', 'ไม่เป็นไร')
  : formatter.quickReplyTemplate('Hi there! We\'re tickets agent. You can try type "event" for listing an events.', 'Event', 'Nothing')

const welcomeBackMessage = (languageCode, formatter) => languageCode === 'th'
  ? formatter.quickReplyTemplate('เป็นไงบ้าง เหงาป่าว? หาอะไรทำมั๊ย', 'มีงานอีเว้นท์อะไรบ้าง', 'เปล่า')
  : formatter.quickReplyTemplate('Welcome back, how can I help you today?', 'Event', 'Nothing')
